...
export async function POST(req: NextRequest) {
  try {
    ...
    // Rate limit: 5 checkout attempts per minute per IP (same as Stripe)
    const rateLimit = checkRateLimit(`paypal-checkout:${ip}`, {
      maxRequests: 5,
    ...
    // Expand plugins (especially bundle)
    const expandedPlugins = cartItems.flatMap((item: { plugin: { id: string, name: string } }) => {
    ...
    const paypalItems = cartItems.map((item: { plugin: { name: string }, payAmount: number }) => {
    ...
    const paidItems = paypalItems.filter((item: { unit_amount: { value: string } }) =>
      parseFloat(item.unit_amount.value) > 0
    );
