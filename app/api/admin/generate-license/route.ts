...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if user has any VocalFelt orders
    const ordersResult = await sql`
      SELECT id, license_key, plugins
      FROM orders
      WHERE email = ${email}
      AND plugins LIKE '%"id":"7"%'
    ...
    if (ordersResult.rows.length === 0) {
      return NextResponse.json(
        { error: 'No VocalFelt purchase found for this email address' },
        { status: 404 }
      );
    }
    ...
    const newLicenseKey = generateVocalFeltLicense();

    // Update the order with the new license
    await sql`
      UPDATE orders
      SET license_key = ${newLicenseKey}
      WHERE id = ${order.id};
    `;
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: 'Your Replacement VocalFelt License Key',
      html: `
        <div style="font-family: monospace; max-width: 600px; margin: 0 auto;">
          <div style="background: #000080; color: white; padding: 15px; border: 4px solid black;">
            <h1 style="margin: 0; font-size: 24px;">VocalFelt License Key</h1>
          </div>
          ...
          <div style="border: 4px solid black; border-top: none; padding: 30px; background: white;">
            <p style="font-size: 16px; line-height: 1.8; margin: 0 0 20px 0;">
              Here is your replacement VocalFelt license key:
            </p>

            <div style="background: #FFD700; border: 4px solid black; padding: 20px; margin: 20px 0;">
            ...
            <div style="background: #f0f0f0; border: 2px solid black; padding: 20px; margin: 20px 0;">
              ...
              <ol style="margin: 0; padding-left: 20px; line-height: 1.8;">
                <li>Download and install VocalFelt</li>
                <li>Create folder: <code style="background: #ddd; padding: 2px 6px;">~/Library/Application Support/TurntPlugins/</code></li>
                <li>Create file: <code style="background: #ddd; padding: 2px 6px;">VocalFelt.lic</code> in that folder</li>
